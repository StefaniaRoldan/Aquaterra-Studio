...
function Calendar() {
  const { reservas, reservar } = useContext(ReservaContext);

  return (
    <div className="grid gap-4">
      {clases.map((clase) => {
        const estaReservada = reservas.includes(clase.id);
        const noHayCupos = clase.cupos === 0;
        ...
        return (
          ...
          >
            <div>
              ...
              <p className="text-sm">
                Cupos: {noHayCupos ? "Sin lugar" : clase.cupos}
              </p>
            </div>
            <button
              disabled={noHayCupos || estaReservada}
              onClick={() => reservar(clase.id)}
              className={`px-4 py-2 rounded ${
                noHayCupos || estaReservada
